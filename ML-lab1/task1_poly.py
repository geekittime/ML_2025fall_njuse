import pandas as pd
import numpy as np
from sklearn.linear_model import LinearRegression  # 修改点：导入线性回归
from sklearn.preprocessing import PolynomialFeatures, StandardScaler  # 修改点：导入多项式特征和标准化工具
...
print(f"移除TTC_hours > 1000的极端离群值后，剩余 {len(merged_df)} 行。")
...
# --- 修改点：为多项式回归创建特征，并进行标准化 ---
print("\n步骤 3.5: 创建多项式特征并进行标准化...")

# 1. 创建多项式特征
# degree=2 表示最高2次幂。注意：degree=3 可能会产生过多特征导致内存问题
poly = PolynomialFeatures(degree=2, include_bias=False)
X_train_poly = poly.fit_transform(X_train)
X_test_poly = poly.transform(X_test)
print(f"创建了 {X_train_poly.shape[1]} 个多项式特征。")
...
# 使用转换并标准化后的测试集进行预测
predictions_log = model.predict(X_test_poly_scaled)
