...
# --- 修改点：定义 Deep&Cross (DCN) 模型 ---
# 1. 先定义核心的 Cross Network 部分
class CrossNetwork(nn.Module):
    def __init__(self, input_dim, num_cross_layers):
        ...
        self.cross_weights = nn.ParameterList([
            nn.Parameter(torch.randn(input_dim, 1)) for _ in range(self.num_layers)
        ])
        self.cross_biases = nn.ParameterList([
            nn.Parameter(torch.randn(input_dim)) for _ in range(self.num_layers)
        ])
    ...
    def forward(self, x):
        ...
        for i in range(self.num_layers):
            # 计算 x_l * w_i^T
            xl_w = torch.matmul(xl, self.cross_weights[i])
            # 计算 x_0 * (x_l * w_i^T) + b_i + x_l
            xl = x0 * xl_w + self.cross_biases[i] + xl
        return xl


# 2. 定义包含 Cross 和 Deep 两部分的完整DCN模型
class DCNClassifier(nn.Module):
    def __init__(self, input_dim, deep_dims=[128, 64], num_cross_layers=3):
        ...
        # 最终的分类器
        # 拼接 Cross 部分的输出(input_dim)和 Deep 部分的输出(deep_dims[-1])
...
# prepare_classification_data, train_classifier, eval_classifier, main 函数与 Wide&Deep 版本中的完全相同
# 我们只需要在 main 函数中实例化 DCNClassifier 即可
...
def train_classifier(X_train_scaled, y_train, X_test_scaled, y_test, input_dim, project_name, device):
    ...
    # --- 修改点：实例化 DCNClassifier 模型 ---
    model = DCNClassifier(input_dim).to(device)
    ...
    print("模型训练完成。")
    model_filename = f'task2_deepcross.pth'
...
def main():
    """主函数 (只需修改模型文件名)"""
    parser = argparse.ArgumentParser(description="使用PyTorch DCN模型完成PR分类任务")
    ...
    if args.do_eval:
        if model is None:
            # --- 修改点：加载 DCNClassifier 模型 ---
            model = DCNClassifier(input_dim).to(device)
            model_path = f'task2_deepcross.pth'
