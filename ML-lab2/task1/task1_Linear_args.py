...
def prepare(project_name="yii2"):
    """加载和预处理数据，返回训练/测试数据和特征维度"""
...
def train(X_train_scaled, y_train, X_test_scaled, y_test, input_dim, project_name, device):
    # 转 tensor
    ...
    model_filename = f'task1_nn_model_{project_name}.pth'
    torch.save(model.state_dict(), model_filename)
    print(f"\n模型已保存到 -> {model_filename}")
    return model

def eval(model, X_test_scaled, y_test, device):
    X_test_tensor = torch.tensor(X_test_scaled.astype(np.float32)).to(device)
    model.eval()
    with torch.no_grad():
    ...
    print("\n--- 性能评估 ---")
    print(f"MAE: {mae:.2f} 小时 (~{mae/24:.2f} 天)")
...
def main():
    ...
    parser.add_argument("--project_name", type=str, default="yii2", help="Project name.")
    args = parser.parse_args()

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    X_train_scaled, X_test_scaled, y_train, y_test, input_dim = prepare(args.project_name)

    model = None
    if args.do_train:
        model = train(X_train_scaled, y_train, X_test_scaled, y_test, input_dim, args.project_name, device)

    if args.do_eval:
        if model is None:  # 没训练就直接加载
            model = MLPRegressor(input_dim).to(device)
            model.load_state_dict(torch.load('task1_Linear.pth', map_location=device))
        eval(model, X_test_scaled, y_test, device)
