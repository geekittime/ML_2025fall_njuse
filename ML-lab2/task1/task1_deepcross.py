...
class CrossLayer(nn.Module):
    ...
    def forward(self, x0, xl):
        # x0: 初始输入, xl: 上一层输出
        # (batch, dim) @ (dim,1) => (batch,1)，再广播 * x0
        cross = torch.matmul(xl, self.weight)  # (batch,1)
        cross = x0 * cross + self.bias + xl   # 残差连接
        return cross


class DeepCrossModel(nn.Module):
    def __init__(self, input_dim, num_cross_layers=3, hidden_units=[128, 64]):
        ...
        # 最终输出层 (Cross 和 Deep 拼接)
        self.final = nn.Linear(input_dim + hidden_units[-1], 1)
...
model = DeepCrossModel(input_dim).to(device)
# print(model)
...
# --- 7. 模型保存 (保持不变) ---
model_filename = 'task1_deepcross.pth'
